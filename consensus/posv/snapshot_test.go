...
func TestNewSnapshot(t *testing.T) {
	...
	sigcache := lru.NewCache[common.Hash, []byte](128)
...
// ========================================================================
// Mainnet Data Dump/Load Utilities
// ========================================================================
// SnapshotDump represents a serializable snapshot for testing with mainnet data
type SnapshotDump struct {
	Number  uint64               `json:"number"`
	Hash    string               `json:"hash"`
	Signers []string             `json:"signers"`
	Recents map[string]string    `json:"recents"` // block number -> signer address
	Votes   []VoteDump           `json:"votes"`
	Tally   map[string]TallyDump `json:"tally"`
	Config  *PosvConfig          `json:"config"`
}
...
// LoadSnapshotFromDump imports a snapshot from JSON-serializable format
func LoadSnapshotFromDump(dump *SnapshotDump) (*Snapshot, error) {
	sigcache := lru.NewCache[common.Hash, []byte](inmemorySnapshots)

	snap := &Snapshot{
		config:   dump.Config,
		sigcache: sigcache,
		Number:   dump.Number,
		Hash:     common.HexToHash(dump.Hash),
		Signers:  make(map[common.Address]struct{}),
		Recents:  make(map[uint64]common.Address),
		Votes:    make([]*clique.Vote, 0, len(dump.Votes)),
		Tally:    make(map[common.Address]clique.Tally),
	}

	// Import signers
	for _, signerHex := range dump.Signers {
		snap.Signers[common.HexToAddress(signerHex)] = struct{}{}
	}

	// Import recents
	for blockNumStr, signerHex := range dump.Recents {
		var blockNum uint64
		fmt.Sscanf(blockNumStr, "%d", &blockNum)
		snap.Recents[blockNum] = common.HexToAddress(signerHex)
	}

	// Import votes
	for _, voteDump := range dump.Votes {
		snap.Votes = append(snap.Votes, &clique.Vote{
			Signer:    common.HexToAddress(voteDump.Signer),
			Block:     voteDump.Block,
			Address:   common.HexToAddress(voteDump.Address),
			Authorize: voteDump.Authorize,
		})
	}

	// Import tally
	for addrHex, tallyDump := range dump.Tally {
		snap.Tally[common.HexToAddress(addrHex)] = clique.Tally{
			Authorize: tallyDump.Authorize,
			Votes:     tallyDump.Votes,
		}
	}
...
func LoadSnapshotFromFile(filename string) (*Snapshot, error) {
	...
	return LoadSnapshotFromDump(&dump)
}

// TestMainnetSnapshot_Block900 tests with real Viction mainnet data from block 900
func TestMainnetSnapshot_Block900(t *testing.T) {
	snap, err := LoadSnapshotFromFile("testdata/snapshot_block_900.json")
	if err != nil {
		if os.IsNotExist(err) {
			t.Skip("Mainnet snapshot file not found")
		}
		t.Fatalf("Failed to load mainnet snapshot: %v", err)
	}

	// Verify this is a checkpoint block
	if snap.Number%snap.config.Epoch != 0 {
		t.Errorf("Block %d is not a checkpoint (epoch=%d)", snap.Number, snap.config.Epoch)
	}

	// Test GetSigners returns sorted addresses
	signers := snap.GetSigners()
	for i := 0; i < len(signers)-1; i++ {
		if bytes.Compare(signers[i][:], signers[i+1][:]) >= 0 {
			t.Error("Signers not properly sorted")
			break
		}
	}

	// Test inturn logic with real signers
	for i, signer := range signers {
		blockNum := snap.Number + uint64(i)
		if !snap.inturn(blockNum, signer) {
			t.Errorf("Block %d: signer %s should be inturn", blockNum, signer.Hex())
		}
	}

	// Verify specific mainnet signers are present
	expectedSigners := []string{
		"0x1b82c4bf317fcafe3d77e8b444c82715d216afe8",
		"0x45b7bd987fa22c9bac89b71f0ded03f6e150ba31",
		"0xad670b2b166684657ffff95f4810380ae7381e9b",
		"0xb112c8f58ad130d90e0a4d9fb3f5c1b75c401250",
		"0xce41231d5dd8cdd7499e418b648c00af75d184a2",
		"0xf9aba09a6fa4a46fb1a6a3919b027d9cac5aa689",
	}

	for _, expectedAddr := range expectedSigners {
		addr := common.HexToAddress(expectedAddr)
		if _, ok := snap.Signers[addr]; !ok {
			t.Errorf("Expected signer %s not found in snapshot", expectedAddr)
		}
	}

	// At checkpoint, votes and tally should be reset
	if len(snap.Votes) != 0 {
		t.Errorf("Expected votes to be cleared at checkpoint, got %d votes", len(snap.Votes))
	}

	if len(snap.Tally) != 0 {
		t.Errorf("Expected tally to be cleared at checkpoint, got %d entries", len(snap.Tally))
	}

	t.Logf("Mainnet snapshot block 900: %d signers, hash=%s", len(signers), snap.Hash.Hex())
}

// TestMainnetSnapshot_SignerRotation tests signer rotation with real mainnet data
func TestMainnetSnapshot_SignerRotation(t *testing.T) {
	snap, err := LoadSnapshotFromFile("testdata/snapshot_block_900.json")
	if err != nil {
		if os.IsNotExist(err) {
			t.Skip("Mainnet snapshot file not found")
	...
	signers := snap.GetSigners()
	numSigners := uint64(len(signers))

	// Test that each signer gets their turn in order
	for i := uint64(0); i < numSigners*2; i++ {
		blockNum := snap.Number + i
		expectedSignerIdx := i % numSigners
		expectedSigner := signers[expectedSignerIdx]

		if !snap.inturn(blockNum, expectedSigner) {
			t.Errorf("Block %d: expected signer %s (index %d) to be inturn",
				blockNum, expectedSigner.Hex(), expectedSignerIdx)
		}

		// Verify other signers are NOT inturn
		for j, signer := range signers {
			if uint64(j) != expectedSignerIdx {
				if snap.inturn(blockNum, signer) {
					t.Errorf("Block %d: signer %s should not be inturn",
						blockNum, signer.Hex())
				}
			}
		}
	...
	t.Logf("Verified signer rotation for %d signers across %d blocks",
		numSigners, numSigners*2)
}

// TestMainnetSnapshot_ValidVote tests voting logic with real mainnet signers
func TestMainnetSnapshot_ValidVote(t *testing.T) {
	snap, err := LoadSnapshotFromFile("testdata/snapshot_block_900.json")
	if err != nil {
		if os.IsNotExist(err) {
			t.Skip("Mainnet snapshot file not found")
	...
	// Test with actual mainnet signers
	signers := snap.GetSigners()
	if len(signers) == 0 {
		t.Fatal("No signers in snapshot")
	}

	existingSigner := signers[0]
	newAddress := common.HexToAddress("0x9999999999999999999999999999999999999999")

	// Should be able to deauthorize existing signer
	if !snap.validVote(existingSigner, false) {
		t.Error("Should allow deauthorizing existing signer")
	}

	// Should NOT be able to authorize existing signer
	if snap.validVote(existingSigner, true) {
		t.Error("Should not allow authorizing existing signer")
	}

	// Should be able to authorize new address
	if !snap.validVote(newAddress, true) {
		t.Error("Should allow authorizing new address")
	}

	// Should NOT be able to deauthorize non-signer
	if snap.validVote(newAddress, false) {
		t.Error("Should not allow deauthorizing non-signer")
	}

	t.Logf("Verified voting logic with %d mainnet signers", len(signers))
}

// TestMainnetSnapshot_DatabasePersistence tests storing/loading with real data
func TestMainnetSnapshot_DatabasePersistence(t *testing.T) {
	snap, err := LoadSnapshotFromFile("testdata/snapshot_block_900.json")
	if err != nil {
		if os.IsNotExist(err) {
			t.Skip("Mainnet snapshot file not found")
	...
	// Load it back
	loaded, err := loadSnapshot(snap.config, snap.sigcache, db, snap.Hash)
	if err != nil {
		t.Fatalf("Failed to load snapshot from db: %v", err)
	}

	// Verify all data preserved
	if loaded.Number != snap.Number {
		t.Errorf("Number mismatch: expected %d, got %d", snap.Number, loaded.Number)
	}
	...
	if len(loaded.Signers) != len(snap.Signers) {
		t.Errorf("Signers count mismatch: expected %d, got %d",
			len(snap.Signers), len(loaded.Signers))
	}

	// Verify each signer is preserved
	for signer := range snap.Signers {
		if _, ok := loaded.Signers[signer]; !ok {
			t.Errorf("Signer %s not found after reload", signer.Hex())
		}
	}

	t.Logf("Successfully persisted and reloaded mainnet snapshot with %d signers",
		len(snap.Signers))
}
